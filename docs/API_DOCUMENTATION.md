...
| `/api/schedule/by-teacher/:id` | По преподавателю | Path: id | Массив занятий с id занятия (idlesson → id) | Массив | — |
| `/api/schedule/by-group/:id` | По группе | Path: id | Массив занятий с id занятия (idlesson → id) | Массив | — |
| `/api/schedule/by-room/:id` | По комнате | Path: id | Массив занятий с id занятия (idlesson → id) | Массив | — |

### Метод POST

| Путь | Описание | Запрос | Ответ | Тип | Права |
|------|----------|--------|-------|-----|-------|
| `/api/schedule/by-date` | Занятия по дате и комнате | date (YYYY-MM-DD), room_id | { success, data } где каждый элемент содержит id занятия (id) | Массив | — |
...
| `/api/schedule` | Удалить занятие (альтернативно) | Body: id или id_schedule | { success, data: { ok } } | Объект | — |
